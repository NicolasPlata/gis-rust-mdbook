...
43. **Hito 11.1 — El contrato de Tokio, IO-bound vs. CPU-bound (2026-09-07):** nueva subsección al inicio de 5.1, con una demostración medida y real del bloqueo del runtime — un servidor Axum con un runtime de un solo hilo (`Builder::new_current_thread()`, para que el efecto no dependa de cuántos núcleos tenga la máquina), un endpoint `/rapido` y uno `/lento` con un cálculo real de `geo` (40M distancias Haversine). Sin `spawn_blocking`, `/rapido` tardó 637.55ms (atrapado detrás de `/lento`, sin relación entre ambos); con `spawn_blocking`, 852.7µs — **748 veces más rápido**, verificado con una aserción de orden de magnitud, no solo impreso. Se añadió una nota corta en 4.7 (donde `async fn main()` aparece por primera vez sin explicación previa) apuntando hacia adelante a esta sección, siguiendo el principio de `CLAUDE.md` de nunca dejar un término sin definir o sin promesa explícita de cuándo se define. Ejercicio 5 nuevo en 5.1, con solución verificada en `soluciones-modulo-4.md` (incluye la aclaración de por qué el runtime multi-hilo por defecto no muestra el efecto con una sola petición lenta). `mdbook build`/`mdbook test` limpios.

44. **Hito 11.2 — Geometrías inválidas y migraciones formales con `sqlx-cli` en 4.5 (2026-09-07):** dos adiciones independientes al mismo capítulo. Primero, la sección "Migración con `ST_SetSRID`" se reescribió como "Migraciones formalizadas con `sqlx-cli`": se instaló `sqlx-cli` 0.8.6 (`cargo install sqlx-cli --version "^0.8" --no-default-features --features rustls,postgres`) y se verificó el ciclo completo (`migrate add -r`, `migrate run`, `migrate info`, `migrate revert`) contra la base real `geoapi_verificacion` — el DDL de `features` ya no vive como texto suelto en el capítulo, vive en `migrations/*.up.sql`/`*.down.sql`. El Ejercicio 1 se reformuló para usar este flujo en vez de `sqlx::query` directo; su solución en `soluciones-modulo-3.md` quedó actualizada con los archivos de migración reales y la salida real de cada comando. Segundo, nueva sección "Rechazar geometrías inválidas antes de insertar": `ST_IsValid`/`ST_IsValidReason` verificados contra PostGIS real con un polígono "bowtie" (`Self-intersection[1 1]`) y un cuadrado válido (`Valid Geometry`); se añadió `insertar_validado` a `FeatureRepositorio` con un nuevo tipo `ErrorRepositorio::GeometriaInvalida`, mapeable a `400 Bad Request` vía RFC 7807 (Capítulo 6.1) — nunca a `500`, porque el problema es de los datos del cliente. Ejercicio 7 nuevo, con tres geometrías inválidas reales y distintas entre sí y de la del capítulo (hueco fuera del exterior → `Hole lies outside shell[5 5]`; dos huecos que se tocan → `Self-intersection[3 3]`; borde autointersecado → `Self-intersection[2 0]`), verificando además que `count(*)` no cambia tras cada rechazo. `mdbook build`/`mdbook test` limpios.
