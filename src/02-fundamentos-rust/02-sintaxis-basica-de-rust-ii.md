...
Si sientes que este capítulo y el anterior fueron demasiado rápidos, o simplemente quieres más ejemplos y ejercicios de sintaxis pura antes de seguir, la referencia canónica es *El lenguaje de programación Rust* — la traducción oficial al español del libro del propio proyecto Rust — disponible gratis en [book.rustlang-es.org](https://book.rustlang-es.org/title-page). No es un prerrequisito: 2.1 y 2.2 ya te dan todo lo que el resto de este libro necesita — es, simplemente, la mejor puerta de entrada si quieres profundizar por tu cuenta.
