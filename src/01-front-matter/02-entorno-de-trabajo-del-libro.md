...
Si ves ese `Finished` sin errores, tu entorno de trabajo está listo. `geoapi-api/src/main.rs` todavía trae el `"Hello, world!"` que genera `cargo new` por defecto — lo vamos a reemplazar en el Capítulo 2.6, cuando tengamos el vocabulario de Rust (`Result`, structs, parseo) para escribir el CLI real de GeoAPI v0.1.
