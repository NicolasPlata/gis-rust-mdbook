...
> **Alerta — dimensionalidad Z/M.** `GEOMETRY(Geometry, 4326)`, tal como está declarada arriba, es una columna **2D**: acepta X/Y, pero no una tercera coordenada de elevación (Z) ni una dimensión de medida lineal (M). Esto es consistente con `geo-types` (la versión 0.7.x que fija este libro), que tampoco representa Z/M — cada `Coord<f64>` es, siempre, un par `(x, y)`. Mientras GeoAPI solo maneja features vectoriales 2D (el caso de todo este módulo), no hay ningún problema. Pero en el Capítulo 4.7 vas a leer nubes de puntos LiDAR con coordenadas `x, y, z` reales — si alguna vez decides persistir esos puntos en una tabla como la de arriba, PostGIS **trunca la coordenada Z en silencio**: no rechaza el insert, no avisa, simplemente la descarta, porque la columna nunca prometió guardarla. Para persistir geometría 3D de verdad, la columna tiene que declararse explícitamente como `GEOMETRYZ` (por ejemplo, `GEOMETRY(PointZ, 4326)`) — y en ese punto, `geo-types` deja de alcanzar: necesitarías un tipo propio (o extender el `struct` con un campo `z: f64` separado) para llevar esa tercera coordenada desde Rust hasta la base de datos sin perderla en el camino.
