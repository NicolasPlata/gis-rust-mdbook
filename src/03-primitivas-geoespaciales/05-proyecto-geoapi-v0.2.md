...
Todo lo que puede fallar en `geoapi-core` — GeoJSON malformado, una geometría que no se puede convertir — se reporta con un único tipo de error, siguiendo el mismo principio del Capítulo 2.4: nunca panic sobre datos externos, siempre `Result` con una variante específica.
