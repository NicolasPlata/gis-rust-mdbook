...
## Post-publicación: Fases 8–14 (mejora continua)
...
- **Fase 14 — Tercera auditoría técnica y editorial** (`docs/plan-fase14-tercera-auditoria.md`): una tercera auditoría externa encontró cinco vacíos, los cinco verificados como genuinos contra el contenido real del libro: (1) `async`/`.await`/Tokio usados desde el Capítulo 4.6 (numeración previa a esta fase: 4.5) sin explicar nunca `Future`, el modelo de *polling*, ni por qué hace falta un runtime; (2) sin patrón de configuración tipada (12-factor), solo `std::env::var()` suelto; (3) TDD local dependiente de una PostGIS levantada a mano, sin `testcontainers-rs`; (4) sin streaming de *entrada* para uploads grandes (`Multipart`), solo un límite de tamaño de payload; (5) sin alerta sobre pérdida de Z/M al persistir geometrías 3D de LiDAR en PostGIS 2D. **Tercera fase post-publicación que renumera capítulos, esta vez dentro del Módulo 3 (4.0):** nuevo Capítulo 4.5 "Fundamentos de Async Rust y Tokio", insertado justo antes del primer uso real de async (Persistencia con PostGIS), corriendo el resto del módulo de 4.5–4.7 a 4.6–4.8. Los otros cuatro hallazgos se resolvieron como secciones nuevas en capítulos ya existentes (6.1, 6.5, 6.6) sin renumerar nada. Ningún otro módulo (1.0–3.0, 5.0–7.0) cambió de numeración.

Las Fases 8–11 no introdujeron capítulos nuevos ni renumeraron nada; las Fases 12–14 sí renumeran, cada una dentro de un solo módulo (12 y 13 en el Módulo 1, 14 en el Módulo 3). Ninguna contradijo el principio de "cero teoría nueva" del Módulo 7.0. Las secciones de abajo ya incorporan todos estos cambios.
...
### 4.5 Capítulo: Fundamentos de Async Rust y Tokio
- **Entregable:** capítulo de referencia rápida, sin contenido GIS todavía. Añadido en la Fase 14, justo antes del primer uso real de async (4.6, Persistencia con PostGIS) y después del último capítulo 100% síncrono del módulo (4.4).
- **Contenido fuente:** ninguno directo en la ruta original — cierra el vacío detectado por una tercera auditoría externa entre la promesa de "cero experiencia previa" del Capítulo 1.1 y el uso de `async`/`.await`/Tokio que el libro hacía desde el Capítulo 4.6 (numeración previa a esta fase: 4.5) sin explicarlo nunca.
- **Densidad de ejercicios:** 3 ejercicios (medir secuencial vs. concurrente con `tokio::join!`, escribir un `Future` manual con `Poll::Pending`/`Waker`, identificar puntos de cesión de control en un fragmento dado).

### 4.6 Capítulo: Persistencia con PostGIS — SQLx y Diesel
- **Contenido fuente:** Fase 2.4.
- **Densidad de ejercicios:** 7 ejercicios (migración formal con `sqlx-cli`, insert vía SQLx con `geozero`, query espacial `ST_DWithin`, mismo flujo con Diesel, índice GiST y medición de mejora, patrón repository, rechazar geometrías inválidas con `ST_IsValid` — el primero reformulado y el último añadido en la Fase 11, con las subsecciones de migraciones formalizadas y geometrías inválidas que los motivan). Añade en la Fase 14 una nota editorial sobre dimensionalidad Z/M (`geo-types` es 2D-only; persistir un punto LiDAR sin una columna `GEOMETRYZ` trunca la elevación en silencio).

### 4.7 Capítulo: I/O adicional — `gdal`, `ndarray`, `shapefile`, `las`
...
### 4.8 Proyecto guiado de cierre de módulo — GeoAPI v0.3
